...
export default function Animal() {
  ...
  useEffect(() => {
    const fetchAnimals = async () => {
      ...
      {if(sortOption == "defaultSortOption") return;}
  ...
  const handleUpdateSave = () => {
    ...
    fetch(`${URL_FETCH_AZURE_SERVER}trainer/update-animal`, {
      ...
      .then((data) => {
        setOpen(false);
        fetch(
          `${URL_FETCH_AZURE_SERVER}trainer/get-animal/${animalDto.animalId}`,
          {
            method: "GET",
            headers: {
              Accept: "application/json",
          ...
          .then((data) => {
            ...
            setAnimals(
              animals.map((animal) => {
                if (animal.animalId === animalDto.animalId)
  ...
  return (
    ...
    >
      ...
      <TableContainer component={Paper} sx={{ mt: "100px" }}>
        ...
        <Button sx={{ float: "right" }} onClick={() => defaultList()}>Back to Default</Button>
        <Typography style={{color: "green", marginLeft: "10px"}}>Direction: {sortDirection == "asc" ? "Ascending" : "Descending"}</Typography>
        <DataTable
          columns={columns}
